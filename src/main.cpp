...
unsigned int nStakeTargetSpacing = 30;			// 15 sec block spacing

int64 nChainStartTime = 1403700264;
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
	...
	if(nHeight == 1)
	{
		nSubsidy = 1800000 * COIN;	// 1800000 coins premine, 4.95% of the total coins created on the first 3 day
		return nSubsidy + nFees;
	}
	
	else if(nHeight > 1 && nHeight <= 8640)
	{
		nSubsidy = 4000 * COIN;	// 4000 coins per block for 3 days
		return nSubsidy + nFees;
	}
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in xxxcoin
    bool fStrictPayToScriptHash = true; // Always active in xxxcoin
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "xxxcoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "xxxcoin a new coin";
        CTransaction txNew;
		...
		block.print();
        assert(block.hashMerkleRoot == uint256("0xa0ca6bc91238acd766c63cdbae3085fd7b1230edfaf40aa4ec5c12398877af3a"));
...
unsigned char pchMessageStart[4] = { 0xce, 0xfb, 0xeb, 0xdb };
